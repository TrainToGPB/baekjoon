...
num_dummies = int(input())
dummies = [int(input()) for _ in range(num_dummies)]
heapify(dummies)

min_comparison = 0
for dummy_idx in range(num_dummies - 1):
    previous = heappop(dummies)
    current = heappop(dummies)
    min_comparison += previous + current
    heappush(dummies, previous + current)

print(min_comparison)
