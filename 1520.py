...
board_dp = [[0] * width for _ in range(height)]
board_dp[0][0] = 1
for row in range(height):
    for col in range(width):
        ...
        if row == 0 and col > 0:
            if board[row][col] < board[row][col-1] and board_dp[row][col-1] > 0:
                board_dp[row][col] = 1
        elif row > 0 and col == 0:
            if board[row][col] < board[row-1][col] and board_dp[row-1][col] > 0:
                board_dp[row][col] = 1
        ...
        for direction in directions:
            ...
            if col == width-1:
                if direction == (-1, 0) or direction == (0, 1):
                    continue
            if board[row][col] < board[row + direction[0]][col + direction[1]]:
                board_dp[row][col] += board_dp[row + direction[0]][col + direction[1]]
        

if board_dp[-1][-1] < board_dp[-2][-1]:
    board_dp[-1][-1] = board_dp[-2][-1]
if board_dp[-1][-1] < board_dp[-1][-2]:
    board_dp[-1][-1] = board_dp[-1][-2]
