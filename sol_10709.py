...
cloud_map = [input() for _ in range(height)]

cloud_time = []
for c_row in cloud_map:
    cloud_time_tmp = []
    for c in c_row:
        if c == 'c':
            cloud_time_tmp.append(0)
        else:
            if 0 in cloud_time_tmp:
                cloud_time_tmp.append(cloud_time_tmp[::-1].index(0) + 1)
            else:
                cloud_time_tmp.append(-1)
    cloud_time.append(cloud_time_tmp)

for ct in cloud_time:
    print(*ct)