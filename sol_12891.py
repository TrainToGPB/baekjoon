...
dnas = input()
password_rules = {dna: int(limit) for dna, limit in zip('ACGT', list(map(int, input().split())))}

cnt = 0
dna_counter = {dna: dnas[:p].count(dna) for dna in 'ACGT'}
for i in range(p, s + 1):
    possible = True
    for dna, limit in password_rules.items():
        if dna_counter[dna] < limit:
            possible = False
            break
    ...
    if i < s:
        dna_counter[dnas[i - p]] -= 1
        dna_counter[dnas[i]] += 1
