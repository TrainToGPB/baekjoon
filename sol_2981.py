...
def find_sub_numbers(number):
    sub_numbers = []
    for i in range(1, int(number**0.5) + 1):
        if number % i != 0:
            continue
        sub_numbers.append(i)
        if i == number // i:
            continue
        sub_numbers.append(number // i)
    
    return sorted(sub_numbers)


min_num, max_num = min(numbers), max(numbers)
diff_num = max_num - min_num
diff_sub_nums = find_sub_numbers(diff_num)
commons = []
for dsn in diff_sub_nums[1:]:
