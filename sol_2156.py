...
not_drink, first_drink, second_drink = [0] * num_wine, [0] * num_wine, [0] * num_wine
for idx, wine in enumerate(wines):
    if idx == 0:
        first_drink[idx] = wine
        second_drink[idx] = wine
    else:
        not_drink[idx] = max(not_drink[idx-1], first_drink[idx-1], second_drink[idx-1])
        first_drink[idx] = wine + not_drink[idx-1]
        second_drink[idx] = wine + first_drink[idx-1]

max_wine = max(not_drink[-1], first_drink[-1], second_drink[-1])
