...
synergy = []
for _ in range(n):
    synergy_partial = list(map(int, input().split()))
    synergy.append(synergy_partial)

synergy_transpose = []
for i in range(n):
    synergy_transpose_partial = []
    for j in range(n):
        synergy_transpose_partial.append(synergy[j][i])
    synergy_transpose.append(synergy_transpose_partial)

synergy_sum = []
for i in range(n):
    for j in range(i):
        print(i, j, synergy[i][j] + synergy_transpose[i][j])
        synergy_sum.append(synergy[i][j] + synergy_transpose[i][j])

min_diff = 100
for s1, s2 in combinations(synergy_sum, 2):
    diff = abs(s1 - s2)
    if diff < min_diff:
        min_diff = diff
